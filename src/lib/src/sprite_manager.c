...
{
    uint8_t col = (uint8_t)(world_x16 >> 3);
    if (col >= map_width) return 0U;
    return tilemap[(uint16_t)tile_row * map_width + col];
}
                                       ...
                                       uint8_t        num_collide_tiles)
{
    ...
    if (!s || !s->active || !tilemap || !collide_tiles || num_collide_tiles == 0U)
        return 0U;
    ...
    col_start = (uint8_t)(ax16 >> 3);
    col_end   = (uint8_t)((ax16 + aw - 1U) >> 3);
    row_start = (uint8_t)(ay >> 3);
    row_end   = (uint8_t)((ay + ah - 1U) >> 3);

    if (col_start >= map_width) return 0U;
    if (col_end   >= map_width)  col_end  = (uint8_t)(map_width  - 1U);
